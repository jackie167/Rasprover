...
LAUNCH_LOG_FILE="$RUNTIME_LOG_DIR/ros_slam_ekf.log"
...
setsid bash -lc "
  ...
  export ROS_STATIC_PEERS='${ROS_STATIC_PEERS}'
  source '$ROS_WS/install/local_setup.bash'
  exec ros2 launch rasprover_bringup slam_ekf.launch.py
" > "$LAUNCH_LOG_FILE" 2>&1 < /dev/null &

LAUNCH_PID=$!
echo "$LAUNCH_PID" > "$PID_DIR/slam_ekf_launch.pid"

echo "ros slam_ekf stack started"
...
echo "  ekf_node: $(check_process '/robot_localization/lib/robot_localization/ekf_node')"
