...
if [ "$EUID" -ne 0 ]; then
    ...
    echo "Use 'sudo ./ops/setup.sh' instead of './ops/setup.sh'"
    echo "Exiting..."
    exit 1
fi
...
if $use_index; then
  sudo -H -u $USER bash -c "cd \"$PROJECT_DIR\" && source ugv-env/bin/activate && pip install -i https://pypi.tuna.tsinghua.edu.cn/simple -r requirements.txt && deactivate"
else
  sudo -H -u $USER bash -c "cd \"$PROJECT_DIR\" && source ugv-env/bin/activate && pip install -r requirements.txt && deactivate"
fi
...
echo "sudo chmod +x ops/autorun.sh"
