...
start_node \
  ...
  "--ros-args -p wheel_separation_m:=${WHEEL_SEPARATION_M:-${FEEDBACK_WHEEL_SEPARATION_M:-0.52}} -p wheel_yaw_scale:=${WHEEL_YAW_SCALE:-2.80} -p linear_odom_scale:=${LINEAR_ODOM_SCALE:-0.976} -p left_odom_scale:=${LEFT_ODOM_SCALE:-0.990} -p right_odom_scale:=${RIGHT_ODOM_SCALE:-1.000} -p publish_tf:=false --log-level info"
sleep 1
start_node \
  "mux" \
...
start_node \
  ...
  "--ros-args -p channel_type:=serial -p serial_port:=/dev/ttyUSB0 -p serial_baudrate:=460800 -p frame_id:=laser -p inverted:=false -p angle_compensate:=true --log-level info"
sleep 1
start_node \
  "laser_tf" \
  "ros2" \
