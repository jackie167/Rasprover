...
class SimpleOdomFilterNode(Node):
    def __init__(self):
        ...
        self.pose_correction_gain_xy = float(self.declare_parameter("pose_correction_gain_xy", 0.08).value)
        self.pose_correction_gain_yaw = float(self.declare_parameter("pose_correction_gain_yaw", 0.10).value)
        self.max_pose_correction_step = float(self.declare_parameter("max_pose_correction_step", 0.02).value)
        self.max_yaw_correction_step = float(self.declare_parameter("max_yaw_correction_step", 0.08).value)
        self.rotation_suppress_angular_threshold = float(
            self.declare_parameter("rotation_suppress_angular_threshold", 0.5).value
        )
        self.rotation_suppress_linear_threshold = float(
            self.declare_parameter("rotation_suppress_linear_threshold", 0.08).value
        )
        self.rotation_translation_scale = float(
            self.declare_parameter("rotation_translation_scale", 0.10).value
        )
        self.rotation_pose_correction_scale = float(
            self.declare_parameter("rotation_pose_correction_scale", 0.05).value
        )
        ...
        self.get_logger().info(
            "simple_odom_filter_node input=%s output=%s publish_tf=%s use_imu=%s rot_suppress=(wz>%.2f,vx<%.2f)"
            % (
                self.input_odom_topic,
                self.output_odom_topic,
                self.publish_tf,
                self.use_imu,
                self.rotation_suppress_angular_threshold,
                self.rotation_suppress_linear_threshold,
            )
        )
    ...
    def handle_imu(self, msg):
        raw_wz = clamp(msg.angular_velocity.z, -self.max_angular_velocity, self.max_angular_velocity)
        if self.latest_imu_wz is None:
            self.latest_imu_wz = raw_wz
            return
        self.latest_imu_wz = self.imu_angular_alpha * raw_wz + (1.0 - self.imu_angular_alpha) * self.latest_imu_wz
    ...
    def handle_wheel_odom(self, msg):
        ...
        suppress_translation = (
            abs(self.filtered_wz) >= self.rotation_suppress_angular_threshold
            and abs(self.filtered_vx) <= self.rotation_suppress_linear_threshold
        )
        effective_vx = self.filtered_vx * (self.rotation_translation_scale if suppress_translation else 1.0)
        xy_correction_scale = self.rotation_pose_correction_scale if suppress_translation else 1.0

        yaw_mid = self.yaw + 0.5 * self.filtered_wz * dt
        self.x += effective_vx * dt * math.cos(yaw_mid)
        self.y += effective_vx * dt * math.sin(yaw_mid)
        ...
        xy_gain = self.pose_correction_gain_xy * xy_correction_scale
        self.x += clamp(dx_err * xy_gain, -self.max_pose_correction_step, self.max_pose_correction_step)
        self.y += clamp(dy_err * xy_gain, -self.max_pose_correction_step, self.max_pose_correction_step)
        self.yaw = normalize_angle(
            self.yaw
            + clamp(
                yaw_err * self.pose_correction_gain_yaw,
                -self.max_yaw_correction_step,
                self.max_yaw_correction_step,
            )
        )
