...
app_config = AppConfig(str(REPO_ROOT / "config.yaml"))


class ReadLine:
    def __init__(self, s, extra_sensor=False, use_lidar=False):
        self.buf = bytearray()
        self.s = s
    ...
    def read_sensor_data(self):
        if self.sensor_data_ser is None:
            return
        ...
        try:
            ...
            while self.sensor_data_ser.in_waiting > 0:
                ...
                decoded = sensor_readline.decode("utf-8")
                if len(sensor_readline) <= self.sensor_data_max_len:
                    self.sensor_list.append(decoded[:-2])
                else:
                    self.sensor_list.append(decoded[: self.sensor_data_max_len])
                    self.sensor_list.append(decoded[self.sensor_data_max_len : -2])
            if buffer_clear:
...
class BaseController:
    ...
    def __init__(self, uart_dev_set, buad_set):
        ...
        self.rl = ReadLine(self.ser, extra_sensor=self.extra_sensor, use_lidar=self.use_lidar)
